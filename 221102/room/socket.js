...
module.exports = (server) => {
  ...
  // 여러개 넣으면 여러번 실행된다. onclick = function 과 addEventListener("click", function)과 비교하면서 테스트해보고 addEventListener와 on은 같은 기능을 한다고 이해해라.
  ...
  io.on("connection", (ws) => {
    ...
    ws.on("chat", (data) => {
      if (!ws.userId) {
        ...
        ws.emit("chat", { text: "좋은 말할 때 방으로 들어가라" });
      } else {
    ...
    ws.on("room", (data) => {
      if (ws.userId) {
        ...
        // io.to(ws.room).emit("loginInfo", { id: ws.userId }); // to()는 어떤 방에 메세지를 보낸다.
        ws.broadcast.to(ws.room).emit("loginInfo", { id: ws.userId });
        // broadcast -> 나를 제외한 모든 나머지 접속자에게
        // to -> 어떤 방에 속한 사람들에게
      } else {
        ws.emit("chat", { text: "방에 들어오고 말해라" });
      }
    });
  });
};
