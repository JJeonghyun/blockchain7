...
// 브라우저와 OS 관련된 정보가 정의되어있다. 즉, 현재 브라우저나 OS등을 확인할 수 있다.
...
// HTML 구조의 root이다.
console.log(document); // html 구조를 알려주는 것.
console.dir(document); // 데이터 객체 구조를 알려주는 것.
// console.log로 나오지 않는건 dir로 확인해보자.
// console.log 확인 시 html 구조가 나오면 dir로 확인하는 경우가 있다.
// console에 대해서 Node.js에서 다시...
// BOM / DOM feat.MVC(Model View Controller)

// Node, Tag, Element 란 무엇인가 ?
// Tag는 HTML에서 사용하는 명령어의 이름을 뜻한다. ex. html, head, body, div ...
...
// Tag && Element의 차이는 무엇이냐 ? >> Tag는 이름 그자체이다. Element는 객체다. 즉, JavaScript에서 HTML 구조를 수정하거나 내용을 추가하거나 등등에서
// 사용하는 Tag에 대한 객체이다.
...
console.log((document.getElementById("test").style.border = "10px solid red"));
console.log(document.getElementById("test").innerHTML);
console.log(document.getElementById("test").outerHTML);
console.dir(document.getElementById("test"));
// 블록7기.이재혁.style.겉옷 = undefined;
// innerHTML 여는태그와 닫는태그 사이의 데이터를 문자열로 받는다.
// outerHTML 여는태그와 닫는태그를 모두 포함하여 모두 문자열로 받는다.

// 상속 : 상속하는 객체의 정보(프로퍼티, 메서드 모두 포함)를 받는 다른 객체를 만드는 행위
// A = {a, b, c} => B = A를 상속한다. ==> B = {a, b, c}
...
// JavaScript는 기본적으로 Node를 기준으로 하고 있다.
