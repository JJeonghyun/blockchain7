...
function App() {
  const temppArr = [
    { name: "정현", age: 17, number: "1", work: "Front" },
    { name: "선주", age: 1, number: "2", work: "Front" },
    { name: "성진", age: 45, number: "3", work: "Back" },
    { name: "영준", age: 2, number: "4", work: "Back" },
    { name: "정규", age: 10, number: "5", work: "Front" },
    { name: "재일", age: 3, number: "6", work: "Back" },
  ];
  const headData = {
    name: "이름",
    age: "나이",
  ...
  return (
    <div className="App">
      <TempTable temppArr={temppArr} headData={headData} tempHead={tempHead} />
      {/* <table>
        <thead>
          <TempTr isHead={true} tableData={headData} head={tempHead} />
        </thead>
        <tbody>
          {temppArr.map((item, index) => (
