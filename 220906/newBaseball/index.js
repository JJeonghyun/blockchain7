...
// 배열은 배열인데 내부에서 중복을 처리해주는 배열(배열이라는 조금 다르다)이다.
// 사용할 때는 new Set() 를 변수를 정의하여 변수를 사용한다.
// clear(), add(), size
...
function reset() {
  ...
  while (comSet.size < 3) {
    ...
    // 만일 중복되는 수라면 알아서 중복되지 않게 처리해준다.
  }
  //   console.log(comSet);
  comSel = [...comSet];
  // ... 은 스프레드라고 한다.
  // 배열, 객체 등 연속되는 데이터들을 하나하나 분해한다.
  // [...comSet]은 comSet 아이템 하나하나를 분해해서 배열안에 넣어준다.
  // if(comSet.size == 3) [comSet[0]], [comSet[1]], [comSet[2]] 와 같은 말이다.

  console.log(comSel);
  alert("컴퓨터 준비 완료. 게임을 시작하지...");
}

function selectNum() {
  if (!comSel.length) return;
  // 컴퓨터가 준비되지 않은 상태에서 숫자가 선택되지 않음. 게임을 진행하지 않도록
  // 함수 멈춤
  let playerSel = "";
  let strike = 0;
  let ball = 0;
  ...
  while (playerSel.length !== 3) {
    ...
    // 떄문에 playerSel을 빈값으로 정의한다.
    // playerSel에 NaN이 있으면 중간에 숫자가 아닌 문자가 있기 때문에
    // 마찬가지로 playerSel을 빈값으로 정의한다.
  }
  strike = ball = 0; // strike와 ball을 0으로 정의한다.
  comSel.forEach((item, index) => {
    // forEach의 매개변수함수(배열/문자열의아이템, 인덱스(순서)) => {내용}
    if (item == playerSel[index]) strike++;
    // 아이템과 playerSel의 index번째 아이템이 같으면 strike++
