...
class Block extends IBlock {
  ...
  create(_previousBlock, _adjustmentDifficulty, _adjustmentTimestamp, _data) {
    try {
      ...
      // 이전 블록의 hash를 이전블록의 hash로 초기화 한다.
      this.height = height + 1;
      // 기존 블록의 높이(개수) 보다 하나 증가 (생성할 때 높이를 증가 시켜서 생성)
  ...
  getMerkleRoot(_data) {
    ...
    return Array.isArray(_data)
      ...
      : { isError: true, error: "이거 오류" }; // 배열이 아니다.
  }
  ...
  createHash(_block) {
    ...
    return SHA256(
      ...
      Object.entries(_block)
        ...
        // hash 생성 메서드가 때문에 hash를 제외, data는 merkleRoot로 대체됨
        .join("")
      // 배열을 하나의 문자열로 연결
    );
  }
  ...
  getDifficulty({
    ...
    // 높이가 난이도 조절 단위 개수 미만일 경우 최초블록에서 현재까지 (난이도가 0)
    ...
    // 높이가 9까지는 0, 높이가 10 ~ 19까지는 1, 이후에는 각 10 단위마다 아래 코드로 난이도를 조절

    const timeTaken = timestamp - adjustmentTimestamp;
    // 현재 시간과 난이도 조절 단위 개수 이전의 시간의 차를 확인
    // 블록이 생성된 시간 - 10개 이전의 생성 시간 = 시간 차

    // 블록 생성 기준시간 10분 블록 하나 생성
    // 10개 만들면 100분
    const timeExpected =
    ...
    // 설정된 블록 10개 생성 시간보다 0.5배 미만이면 난이도 증가, 300초(5분) 보다 작은경우

    if (timeTaken > timeExpected * 1.5) return adjustmentDifficulty - 1;
    // 설정된 블록 10개 생성 시간보다 1.5배 초과이면 난이도 감소, 900초(15분) 보다 큰 경우
  ...
  updateBlock(_previousBlock, _adjustmentDifficulty, _adjustmentTimestamp) {
    ...
    while (!hashBinary.startsWith("0".repeat(this.difficulty))) {
      ...
      // 비교를 위해서 hash를 다시 bit 형식, binary 형식으로 변경
    }
  }

  // 블록의 검증 함수, 블록이 정상적인지 검사
  isValidNewBlock(_newBlock, _previousBlock) {
    ...
    if (_newBlock.previousHash !== _previousBlock.hash) {
      // 생성된 블록의 저장된 이전 블록의 hash가 이전 블록 hash와 같은지 확인
