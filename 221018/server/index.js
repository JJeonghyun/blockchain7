...
// 환경설정 파일을 로드하기 위한 라이브러리
const path = require("path");
// 경로 내장 모듈
const cookieParser = require("cookie-parser");
// 쿠키를 위한 라이브러리 < 세션과 같이 나중에
const cusMath = require("./cusMath").sum;

console.log(cusMath(1, 2));
...
//  쿠키 허락?
//  사용자의 정보를 임시 저장한다.
//  원래 쿠키는 데이터를 임시 저장한 공간, 여기서 데이터는 간단한 텍스트 < 이미지, 영상 등등은 포함되지 않는다
//  쿠키 VS 캐시
//  cache 캐시 < 이미지, 영상 등을 임시 저장하는 공간, 캐시는 파일로 저장된다

// 세션이란? 쿠키랑 같은 기능을 한다. < 텍스트를 임시로 저장한다
//  쿠키는 로컬에 저장한다. 브라우저에서 관리한다
//  세션은 서버에 저장한다. 서버쪽에서 관리한다
...
// console.log(cusMath.multiplay(3, 4));
