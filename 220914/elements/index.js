...
// children은 엘리먼드의 자식 엘리먼트를 가져온다, 호출한다.
...
let children = [...document.getElementById("parent").children]; // << 얘는 배열이 아니라 컬렉션이다. 단, forEach를 사용하기 위해 스프레드(...)를 통해 배열로 바꿔준다.
...
[...children].forEach(function (elem, index) {
  ...
  // forEach는 배열의 아이템을 하나하나 가져와서 매개변수함수로 전달된 함수에 매개변수로 전달해서
  // 전달해서 함수를 호출한다.
  elem.onmouseover = () => {
  ...
  elem.onclick = function () {
    ...
    /*{
      ...
      // contains 메서드는 매개변수로 전달된 문자열(string)이 클래스에 포함되어 있는지 확인한다.
