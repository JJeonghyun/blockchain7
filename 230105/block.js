...
class Header {
  constructor(_height, _previousHash) {
    ...
    // 이전 블록의 해시값
    // 최초 블록은 이전 블록의 해시값이 없으니까 값이 없으면 0으로 만들려고 문자열을 넣어준다
    this.previousHash = _previousHash || "0".repeat(64);
  }
  // static으로 만들어서 전역으로 사용할 수 있는 함수
  // class 동적할당하는데 일반적인 함수로 만들면 생성된 객체의 함수
  // class 자체에 static 함수가 포함되어있어서 클래스를 호출할 때 마다 static으로 선언된 함수가 만들어지는 데이터 낭비가 이루어지지 않는다
...
// 예를 들어 일반적인 함수는 New로 동적할당 할 때 마다 C라는 함수가 있으면
// 동적할당한 A,B에 둘 다 생성한 객체안에 함수가 들어있지만 static으로 선언하면
// 클래스에 전역 함수 하나만 있게 된다. 동적 할때 마다 생성 될 필요가 없는 함수는 static으로 선언해주는 것이 좋다
...
// block Header 생성
// 최초 블록(제네시스블록)이라 이전 블록에 대한 정보는 담지 않는다.
...
const secondBlock = new Block(secondHeader, ["난 두번쨰임"]);
