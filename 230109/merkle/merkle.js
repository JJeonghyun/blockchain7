...
// merkle 라이브러리는 받은 데이터 배열의 아이템을 전부 문자열 (string)으로 처리한다

const createMerkleRoot = () => {
  return SHA256(data[0].toString()).toString().toUpperCase();
  //   return SHA256(`${data[0]}`).toString().toUpperCase();
  // SHA256(data[0]) << 데이터를 암호화 한다. 해당 암호화된 데이터는 객체 형식이다
  // .toString() << 문자열로 변환한다.
  // .toUpperCase() << SHA256를 쓸 경우 영어가 소문자로 나타나기에 대문자로 전부 변환해준다
};
...
// 데이터의 모든 정보(아이템)을 한번씩 hash(SHA256) 방식으로 암호화 한다.

const secondTreeRoot = SHA256(firstTree[0] + firstTree[1])
  .toString()
  .toUpperCase();
// 데이터(data) 안의 2개의 아이템을 암호화한 후 연결해서 다시 암호화하여 한개의 문자열로 만든다
