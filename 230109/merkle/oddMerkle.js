...
for (let i = 0; i < firstTree.length; i += 2) {
  let temp = ""; // 이름 그대로 임시값을 준다. 아래에서 조건에 따라 값을 정의
...
const createMerkleRoot = (_data) => {
  ...
  if (!Array.isArray(_data)) return { isError: true, msg: "너 배열 아니다.." };
  // isArray : Array(배열 객체)의 메서드로 배열인지 아닌지를 판단한다
  //    배열이라면 true 준다. (반환한다. return한다.)
  // return을 객체로 내보내는 이유 : 블록 생성 후 해당 블록이 정상적인 블록인지 확인하기 위해서
  // 객체로 내보낸다. isError를 통해서 생성 도중 문제가 발생했는지를 먼저 파악할 수 있다
