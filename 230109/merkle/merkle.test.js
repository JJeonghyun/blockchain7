...
describe("테스트의 컨테이너", () => {
  ...
  // test || it
  // it(name, func)
  // it 또는 test는 name이라는 이름의 테스트를 실행한다
  // 해당 테스트의 구현 내용(코드)는 func(함수)에 포함된다

  // expect(확인할 내용) : 적혀 있는 그대로 확인할 함수, 객체, 변수 등등을 매개변수로 전달한다. 이것만 있으면 아무 의미가 없다.
  // toBe(비교 내용) : 정확히 일치 하는지를 확인한다.
  //  - to****** 를 매쳐(matcher)라고 부른다 < 엄청 많다
  ...
  describe("toEqual VS toStrictEqual", () => {
    // describe는 다른 describe안에 사용할 수 있으며, 하위 목록이 된다
    // Strict : 엄격한
    it("함 드가자", () => {
...
describe("merkle 비교 드가자", () => {
  //   it("두 merkle과 createMerkle 확인", () => {
  //     expect(createRoot).toBe(merkleRoot);
